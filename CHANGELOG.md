...
- Active-branch `originweave-http` bounded HTTP/1.1 exchange slice over an authenticated TLS stream: generated `GET`/`HEAD` requests, strict RFC 9112 response framing, bounded chunked/close-delimited bodies, no-body semantics, credential-free safe headers, and fail-closed ALPN, redirect, coding, timeout, and incomplete-response boundaries. This does not claim protected-main integration or complete HTTP/MIME/download/integrity behavior.
