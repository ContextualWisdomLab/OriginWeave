...
- Controlled pinned-Chromium Agent Task acceptance now fails closed unless the temporary profile is pristine before launch and browser-observed cookies, local storage, session storage, and extension count are all empty, retaining bounded per-trial isolation evidence without claiming OS- or browser-attested absence of every credential mechanism.
