...
### Added
- Capture lifecycle transitions now commit trusted time only after every state, retention, and legal-hold precondition succeeds, preventing rejected transitions from poisoning later lifecycle decisions.
- Active PR #239 adds a policy-neutral `CaptureLifecycle` contract for manifest-bound capture state, trusted-time-monotonic transitions, explicit retention expiry, legal-hold blocking, and deletion confirmation. It remains active-PR evidence only and does not persist or delete artifacts, authenticate operators, decide legal entitlement, or grant capture, replay, export, or secret access.
