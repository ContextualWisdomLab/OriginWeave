...
- Bounded browser-task runtime telemetry that validates and stores platform-supplied browser/task RSS, semantic-observation bytes, governed-action latency, and total task duration without implementing OS/Chromium sampling or inferring GPU/local-AI evidence; supplied browser RSS can feed the resource governor through conservative, overflow-safe byte-to-mebibyte ceiling conversion.
