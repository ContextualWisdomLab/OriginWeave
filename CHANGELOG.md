...
- Deterministic W3C PROV-O JSON-LD projection over validated WARC resource records, binding exact source and WARC digests, capture time, and OriginWeave software revision while preserving complete-versus-truncated payload state and exact standard WARC truncation reason without embedding raw payload bytes.
