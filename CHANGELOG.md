...
### Added
- Added a version-pinned `originweave-bidi` presentation-capability boundary for the 18 August 2026 WebDriver BiDi contract; it depends inward on `originweave-fingerprint` and fails closed on the complete profile because hardware concurrency and the complete Chromium platform/User-Agent Client Hints surface require a separate versioned Chromium adapter. This does not apply a profile to Chromium or prove page-observed post-conditions.
