...
- Credential-safe verified action-outcome evidence that binds action kind, canonical target origin, complete action-intent digest, bounded browser post-condition kind, a same-clock monotonic dispatch-to-observation window with a caller-selected positive freshness budget, and exact provenance; construction rejects unverified or rejected post-conditions, pre-dispatch observations, stale observations beyond that budget, and generic `NodeStateChanged` evidence without the exact governed `ObservedNodeHandle`, while node-state construction additionally requires canonical provenance origin equality and the independently observed node to match the exact governed session/context/origin/document-epoch/node identity; equal coarse-clock ticks and canonical-equivalent source origins remain valid.
...
- Immutable capture-manifest evidence that binds a bounded capture identifier and extraction-schema identifier to one structured-value proof plus canonical SHA-256 identities for its durable WARC record set and provenance graph, without carrying raw extracted values or artifact bytes.
