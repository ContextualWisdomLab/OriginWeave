...
- Separated hourly product PR publication authority from the organization review and merge system, added live default-branch and release-blocker rechecks immediately before publication, query up to 100 release-blocker candidates before filtering pull-request entries so a labeled PR cannot mask a real blocking issue, and made a missing dedicated `OPENCODE_PR_TOKEN` fail closed after a verified change instead of producing a green publication no-op.
