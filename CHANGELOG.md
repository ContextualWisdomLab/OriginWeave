...
- Document-bound node authority with nonzero document epochs and adapter-local node identifiers, exact canonical-origin binding, and deterministic stale-epoch rejection before a future browser adapter acts on an observed node.
...
- Observed node handles cannot be reused after the canonical origin or document epoch changes, preventing stale pre-navigation node authority from silently crossing a document boundary.
