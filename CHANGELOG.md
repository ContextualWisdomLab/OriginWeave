...
### Added
- Clarified ADR 0007's sensitive-handle boundary: `uses_so_far` is untrusted predicate input, the pure predicate does not independently enforce the maximum-use limit, and only a trusted broker or `SensitiveHandleUseState` may treat its result as authorization.
...
- In-process authoritative sensitive-handle use reservation and first-revocation-wins lifecycle state that owns the bounded use count and exact-scope monotonic trusted-time floor, records task-completion/policy-change/key-rotation/session-termination/suspicious-use revocation causes, blocks all future reservations after revocation, increments only after exact scope/classification/expiry/use-limit authorization, keeps scope-mismatched requests from reading or advancing lifecycle/time state, and leaves denied exact-scope reservations unconsumed; this is a policy primitive only and does not claim durable broker storage, protected-value resolution, authenticated audience binding, or cross-process transactionality.
