...
- Standard `Display` and `std::error::Error` contracts for destination, redirect, digest, direct-network, TLS, resource-budget, and evidence failures, including preserved destination-policy, rustls, and operating-system sources where applicable.
...
- Provenance source locators reject control characters and reviewed invisible Unicode formatting characters so credential-free evidence cannot forge line-oriented audit or log presentation.
...
- Network evidence query field names reject control and reviewed invisible Unicode formatting characters while preserving printable Unicode names and keeping query syntax distinct from HTTP token grammar.
