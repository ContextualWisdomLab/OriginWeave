...
- Added the public benchmark-failure taxonomy, fixed-point zero-event threshold evaluator, bounded zero-event safety gate, and `decide_commercial_release_with_zero_event_safety` evidence combiner; missing, invalid, inconclusive, or statistically insufficient mandatory evidence remains non-passing and these APIs do not grant merge, tag, publication, or release authority.
