...
- Deterministic sensitive-data break-glass composition that permits only existing human-approval or dual-control disclosure decisions after exact authority and reason binding, non-transferable current-to-approved actor identity, a positive half-open interval within an explicit local maximum window, sufficient approval evidence, heightened monitoring, and mandatory post-event review; this metadata-only boundary does not authenticate actors or approvers, disclose protected values, execute monitoring, persist audit evidence, or perform the review.
- Request-bound, freshness-bounded necessity-gated full-field sensitive-model disclosure composition that runs exact sensitive-data and reviewed invocation authorization first, rejects necessity evidence bound to another exact tenant/task/field/purpose/destination/classification request, then rejects zero or expired evidence at an exclusive trusted-time horizon; fresh `NoLowerDisclosurePath` may preserve authorization while opaque-handle, deterministic-transform, local-rule, structured-tool, or approved-derived-value alternatives return typed `FullFieldNotNecessary`, and the pure policy boundary does not attest caller truth, clock provenance, or release protected bytes.
...
- Covered the request accessor in the necessity-evidence contract so exact production coverage includes the complete model-disclosure evidence surface.
