...
- Host-managed Agent Task extension admission that default-denies unmanaged extensions, admits exact allow-listed Chromium identifiers as a compatibility surface only, lets an explicit block-list win, and never treats Chrome `force_installed` as an OriginWeave Agent grant.
