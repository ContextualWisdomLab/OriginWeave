...
- Reusable TLS leaf-certificate validity-horizon guard that layers a fail-closed delegated-task safety budget on authenticated fixed-time evidence, conservatively rounds fractional durations, saturates extreme inputs without integer wrap, and keeps the product policy distinct from RFC 5280 point-in-time validity semantics.
