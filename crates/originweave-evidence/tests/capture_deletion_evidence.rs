...
fn retained_lifecycle() -> Result<CaptureLifecycle, CaptureLifecycleError> {
...
fn deletion_confirmation_requires_manifest_bound_evidence_receipt() -> Result<(), Box<dyn Error>> {
    ...
    let foreign_receipt = CaptureDeletionReceipt::new(
...
#[test]
fn deletion_receipt_rejects_unbound_or_noncanonical_identity() {
    assert_eq!(
        CaptureDeletionReceipt::new(
            MANIFEST_DIGEST,
            CURRENT_DELETION_REQUEST_DIGEST,
            "sha256:not-a-digest"
        ),
...
fn deletion_request_rejects_noncanonical_request_identity_atomically() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        lifecycle.request_deletion("sha256:not-a-digest", 200),
        Err(CaptureLifecycleError::InvalidDeletionRequestDigest)
    );
