...
/// policy, approval, secret, network, or adapter authority and does not perform browser I/O.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BrowserProtocolOperation {
    /// Navigate one controlled browser context.
    Navigate,
    /// Produce one bounded semantic browser observation.
    ObserveSemantics,
    /// Dispatch policy-authorized typed browser input.
    DispatchTypedInput,
...
impl BrowserProtocolOperation {
    ...
    pub const fn required_capability(self) -> BrowserProtocolCapability {
        match self {
            ...
            Self::ObserveSemantics => BrowserProtocolCapability::SemanticObservation,
            Self::DispatchTypedInput => BrowserProtocolCapability::TypedInput,
