...
///
/// The runtime protocol family, adapter version, protocol revision, and browser revision must be
/// sampled from the trusted adapter that is about to perform the operation. Validation occurs
/// before `dispatch` is invoked, and the callback receives the resulting non-cloneable
...
///
/// A successful callback invocation does not authenticate the adapter process, authorize a browser
/// session, browsing context, origin, destination, secret, or approval, or prove a browser
/// post-condition. Those remain separate higher-level execution boundaries.
pub fn dispatch_browser_protocol_if_runtime_matches<R, F>(
