...
#[test]
fn browsing_contexts_epochs_and_node_handles_reject_invalid_identifiers() {
    assert_eq!(
    ...
    assert_eq!(
        ObservedNodeHandle::new(browsing_context, origin, epoch, 0),
        Err(NodeHandleError::InvalidNodeId)
    );
}

#[test]
fn a_node_handle_is_valid_only_for_its_exact_context_origin_and_document_epoch() {
    let origin = Origin::parse("https://example.com").expect("canonical origin");
    ...
    assert_eq!(
        handle.validate_current(observed_context, &origin, observed_epoch),
        Ok(())
    );
    assert_eq!(
        handle.validate_current(other_context, &origin, observed_epoch),
    ...
    assert_eq!(
        handle.validate_current(observed_context, &origin, current_epoch),
    ...
    assert_eq!(
        handle.validate_current(observed_context, &other_origin, observed_epoch),
