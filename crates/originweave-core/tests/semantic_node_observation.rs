...
use originweave_core::{
    ...
    BrowserProtocolCapability, BrowserProtocolKind, BrowserSessionId, BrowsingContextId,
    MAX_ACCESSIBLE_NAME_BYTES, MAX_SEMANTIC_CHILDREN, MAX_SEMANTIC_ROLE_BYTES,
    MAX_VISIBLE_TEXT_BYTES, NodeActionKind, ObservationChannel, ObservedNodeHandle, Origin,
    OriginWeaveProtocolVersion, SemanticNodeObservation, SemanticNodeObservationError,
...
impl Fixture {
    ...
    fn bind_authority(
        ...
        WebDriverBiDiAccessibilityQuery::new(Some("generic"), None, 1)
            .map_err(|error| error.to_string())?
            .bind_current_nodes(
                proof,
                &mut self.registry,
            ...
            .into_iter()
            .next()
            .ok_or_else(|| "semantic observation fixture returned no node".to_owned())
    }
...
fn semantic_node_rejects_live_relationships_from_other_authority() -> Result<(), String> {
    ...
    let other_context_node = fixture.bind_authority(
        fixture.session,
        other_context,
        &origin,
        "other-context-node",
    ...
    let other_session_node = fixture.bind_authority(
        other_session,
