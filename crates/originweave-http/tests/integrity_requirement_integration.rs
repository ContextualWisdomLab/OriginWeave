...
fn authenticated_connection(
    ...
    authenticate(origin, direct, root_der)
}
...
fn authenticate(
    ...
    let roots = TrustRootBundle::new(
        TrustBundleIdentifier::parse("http_digest_requirement_loopback:v1")
    ...
    TlsHandshakePlan::new(origin.clone(), connection, roots, policy)
        .expect("TLS handshake plan")
        .authenticate()
        .expect("authenticated loopback TLS")
}

fn required_digest_policy() -> HttpClientPolicy {
    HttpClientPolicy::from_limits(
        TEST_TIMEOUT,
        HttpPolicyLimits::strict_defaults(),
...
fn execute_response_result(
    ...
    let target = HttpRequestTarget::parse(origin, "/digest-contract").expect("target");
...
#[test]
fn required_digest_accepts_a_single_verified_content_digest() {
...
#[test]
fn required_digest_accepts_a_single_verified_representation_digest() {
    let response = execute_response(
