...
enum ServerBehavior {
    WriteAndCloseCleanly(&'static [u8]),
    Stall(Duration),
}
...
fn certificate_authority() -> (Vec<u8>, Issuer<'static, KeyPair>) {
    ...
    parameters.distinguished_name.push(
        DnType::CommonName,
        "OriginWeave HTTP exchange-region test root",
    );
...
fn spawn_http_server(
    ...
    let handle = thread::spawn(move || {
        ...
        match behavior {
            ...
            ServerBehavior::Stall(delay) => thread::sleep(delay),
        }
        Ok(request)
    });
...
fn policy_with_timeout(exchange_timeout: Duration) -> HttpClientPolicy {
    ...
    HttpClientPolicy::new(
        ...
        defaults.alpn_policy(),
        defaults.integrity_requirement(),
    )
...
fn execute(
    ...
    let target = HttpRequestTarget::parse(origin, "/exchange-region").expect("target");
...
fn assert_request(server: JoinHandle<ServerResult>) {
    let request = server
        .join()
        .expect("server thread")
        .expect("server exchange");
    assert!(request.starts_with(b"GET /exchange-region HTTP/1.1\r\n"));
}

#[test]
fn malformed_response_head_propagates_through_the_authenticated_exchange() {
    let response = b"HTTP/1.1 XYZ Nope\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
    let (result, server) = execute(
        ServerBehavior::WriteAndCloseCleanly(response),
        HttpClientPolicy::strict_defaults(),
    );
    assert!(matches!(result, Err(HttpError::InvalidResponseStatusLine)));
    assert_request(server);
}
...
fn malformed_chunked_body_propagates_through_the_authenticated_exchange() {
    ...
    assert!(matches!(result, Err(HttpError::MalformedChunkedBody)));
    assert_request(server);
}
