...
fn certificate_authority() -> (Vec<u8>, Issuer<'static, KeyPair>) {
    ...
    parameters
        .distinguished_name
        .push(DnType::CommonName, "OriginWeave persistent chunked test root");
...
fn spawn_persistent_chunked_server(config: Arc<ServerConfig>) -> (SocketAddr, thread::JoinHandle<Vec<u8>>) {
