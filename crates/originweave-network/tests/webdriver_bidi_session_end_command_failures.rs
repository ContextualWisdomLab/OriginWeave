...
#[test]
fn session_end_rejects_ids_above_the_webdriver_bidi_js_uint_range() {
...
-> Result<(), Box<dyn Error>> {
    ...
    let error = command
        ...
        .err()
        .ok_or_else(|| io::Error::other("duplicate correlation unexpectedly sent session.end"))?;
    assert!(matches!(
        error,
        WebDriverBiDiSessionEndCommandError::Correlation { .. }
    ));
    assert_eq!(correlation.outstanding_count(), 1);

    server
        .join()
        .map_err(|_| io::Error::other("duplicate-correlation session.end server panicked"))??;
    Ok(())
}
