...
#[test]
fn current_node_text_value_observation_is_rejected_before_writing_to_a_foreign_session_transport()
-> Result<(), Box<dyn Error>> {
    ...
    let send_result = send_webdriver_bidi_text_value_observation(
        protocol_proof(BrowserProtocolCapability::SemanticObservation)?,
        42,
        "context-a",
        &handle,
    ...
    let error = send_result.err().ok_or_else(|| {
        io::Error::other(
            "registry session A unexpectedly dispatched text-value observation input on transport session B",
        )
    })?;
    assert!(matches!(
        error,
        WebDriverBiDiTextValueObservationSendError::Authority {
            source: WebDriverBiDiTextValueObservationAuthorityError::BrowserAuthority(_)
        }
    ));
    assert_eq!(
    ...
    assert!(
        !command_byte_seen,
        "foreign-session rejection must happen before any text-value observation command-frame byte"
    );
    Ok(())
}
