...
fn read_masked_text_frame(stream: &mut TcpStream) -> io::Result<Vec<u8>> {
    ...
    if length > 125 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "session.status fixture unexpectedly required extended framing",
        ));
    }
    let mut mask = [0_u8; 4];
...
-> Result<(), Box<dyn Error>> {
    ...
    let original_server = thread::spawn(move || -> io::Result<()> {
        ...
        if command != br#"{"id":7,"method":"session.status","params":{}}"# {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "unexpected session.status command on original connection",
            ));
        }
        Ok(())
    });
    ...
    let _original = WebDriverBiDiSessionStatusCommand::new(7)?.send(
        original,
        &mut correlation,
    ...
    original_server
        .join()
        .map_err(|_| io::Error::other("original-connection server panicked"))??;
    assert_eq!(correlation.outstanding_count(), 1);

    let replacement_listener = TcpListener::bind(("127.0.0.1", 0))?;
    let replacement_response = receive_foreign_status_response(replacement_listener)?;
    let parsed = WebDriverBiDiSessionStatusResult::parse_and_correlate(
    ...
    assert_eq!(
        ...
        1,
        "foreign-connection rejection must leave the original command pending"
    );
    Ok(())
}
