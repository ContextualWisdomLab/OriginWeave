...
pub fn acknowledged_type_text_intent(
    command_id: u64,
    text: &str,
) -> Result<WebDriverBiDiAcknowledgedTypeTextIntent, Box<dyn Error>> {
    let listener = TcpListener::bind(("127.0.0.1", 0))?;
    let local_addr = listener.local_addr()?;
    let expected_prefix =
        format!(r#"{{"id":{command_id},"method":"input.performActions""#).into_bytes();
    let ack = format!(r#"{{"type":"success","id":{command_id},"result":{{}}}}"#).into_bytes();
