...
fn receive_event(payload: &[u8]) -> Result<WebDriverBiDiWebSocketTextMessage, Box<dyn Error>> {
    ...
    let text = match assembler.push_frame(frame)? {
        ...
        other => {
            return Err(io::Error::other(format!(
                "navigation semantic-boundary event produced unexpected assembly state: {other:?}"
            ))
            .into());
        }
    };
    server
        .join()
        .map_err(|_| io::Error::other("navigation semantic-boundary server panicked"))??;
    Ok(text)
}

#[test]
fn navigation_committed_rejects_valid_json_semantic_type_and_js_uint_boundaries_after_transport()
-> Result<(), Box<dyn Error>> {
    let cases: &[&[u8]] = &[
        ...
        br#"{"type":"event","method":"browsingContext.navigationCommitted","params":{"context":"context-a","navigation":null,"timestamp":9007199254740992,"url":"https://example.test/after"}}"#,
    ...
    for payload in cases {
        let event = receive_event(payload)?;
