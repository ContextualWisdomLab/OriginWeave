...
fn read_masked_client_frame(stream: &mut TcpStream, opcode: u8) -> io::Result<Vec<u8>> {
    let mut header = [0_u8; 2];
    stream.read_exact(&mut header)?;
    if header[0] != opcode || header[1] & 0x80 == 0 {
        return Err(io::Error::new(
...
#[test]
fn type_text_command_writes_exact_masked_bidi_frame_and_stays_outstanding()
-> Result<(), Box<dyn Error>> {
    ...
    let _established = send_webdriver_bidi_type_text(
        typed_input_proof()?,
        42,
        "context-a",
        "Quarterly review",
...
#[test]
fn type_text_reused_mask_key_rejection_retires_correlation() -> Result<(), Box<dyn Error>> {
    let listener = TcpListener::bind(("127.0.0.1", 0))?;
    let local_addr = listener.local_addr()?;
    let server = thread::spawn(move || -> io::Result<()> {
        let (mut stream, _) = listener.accept()?;
...
#[test]
fn type_text_ambiguous_socket_write_keeps_correlation() -> Result<(), Box<dyn Error>> {
    let listener = TcpListener::bind(("127.0.0.1", 0))?;
    let local_addr = listener.local_addr()?;
    ...
    let server = thread::spawn(move || -> io::Result<()> {
        ...
        closed_sender
            .send(())
            .map_err(|_| io::Error::other("text-input close signal receiver disappeared"))
    });
    ...
    for attempt in 0_u8..64 {
        ...
        match send_webdriver_bidi_type_text(
            typed_input_proof()?,
            command_id,
            "context-a",
            "Quarterly review",
            ...
            Ok(next) => {
                correlation.retire_command_for(command_id, WebDriverBiDiCommandKind::TypeText)?;
                established = next;
            }
            Err(error) => {
