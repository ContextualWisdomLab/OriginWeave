...
#[test]
fn public_message_admission_consumes_real_validated_frame() -> Result<(), Box<dyn Error>> {
    ...
    let server = thread::spawn(move || -> io::Result<()> {
        ...
        stream.write_all(&[0x81, 0x02, b'o', b'k'])
    });
    ...
    server
        .join()
        .map_err(|_| io::Error::other("validated-frame server panicked"))??;
    Ok(())
}
