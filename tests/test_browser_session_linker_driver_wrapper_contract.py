...
class BrowserSessionLinkerDriverWrapperContractTests(unittest.TestCase):
    """Keep GCC subcommand wrapper authority inside the reviewed Browser Session execution boundary."""
    ...
    def test_repository_split_driver_wrapper_fails_closed(self) -> None:
        root = self._workspace_with_flags(
            '["-C", "link-arg=-wrapper", "-C", "link-arg=tools/review-wrapper,--args"]'
        )
    ...
    def test_repository_driver_wrapper_inside_link_args_fails_closed(self) -> None:
        root = self._workspace_with_flags(
            '["--codegen=link-args=-wrapper tools/review-wrapper,--args"]'
        )
        with self.assertRaisesRegex(AssertionError, "Cargo .*execution override"):
            authority._assert_no_repository_cargo_compiler_execution_overrides(root)
