...
class BrowserSessionLldMllvmAuthorityContractTests(unittest.TestCase):
    """Keep repository-selected LLD-to-LLVM option forwarding outside the reviewed build TCB."""
    ...
    def test_build_rustflags_lld_mllvm_equals_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=-Wl,--mllvm=-debug-pass=Structure"]\n',
            "rustflags:codegen linker",
        )

    def test_target_rustflags_lld_mllvm_split_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=-Wl,-mllvm,-print-after-all\"]\n",
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_lld_mllvm_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,--mllvm=-print-after-all"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_build_arg_lld_mllvm_fails_closed(self) -> None:
        self._assert_fails_closed(
