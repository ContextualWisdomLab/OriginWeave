...
class BrowserSessionHostCpuCodegenAuthorityContractTests(unittest.TestCase):
    """Keep repository-selected host-CPU-dependent code generation out of reproducible builds."""
    ...
    def test_build_rustflags_target_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "target-cpu=native"]\n',
            "rustflags:ambient host cpu",
        )

    def test_target_rustflags_compact_target_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-Ctarget-cpu=native\"]\n",
            "rustflags:ambient host cpu",
        )

    def test_profile_rustflags_target_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[unstable]\nprofile-rustflags = true\n\n[profile.release]\nrustflags = ["--codegen=target-cpu=native"]\n',
            "profile.release.rustflags:ambient host cpu",
        )

    def test_build_rustdocflags_target_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--codegen=target-cpu=native"]\n',
            "rustdocflags:ambient host cpu",
        )

    def test_doctest_build_arg_target_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--doctest-build-arg=-C", "--doctest-build-arg=target-cpu=native"]\n',
            "rustdocflags:doctest compiler authority",
        )

    def test_build_rustflags_tune_cpu_native_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-Z", "tune-cpu=native"]\n',
            "rustflags:ambient host cpu",
        )

    def test_explicit_target_cpu_remains_allowed(self) -> None:
