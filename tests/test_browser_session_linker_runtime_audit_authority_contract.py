...
class BrowserSessionLinkerRuntimeAuditAuthorityContractTests(unittest.TestCase):
    """Keep ELF rtld-audit selection inside reviewed Browser Session provenance."""
    ...
    def test_build_rustflags_audit_library_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=-Wl,--audit=tools/review-bypass-audit.so"]\n',
            "rustflags:codegen linker",
        )

    def test_target_rustflags_depaudit_library_fails_closed(self) -> None:
        self._assert_fails_closed(
    ...
    def test_doctest_forwarded_audit_library_fails_closed(self) -> None:
        self._assert_fails_closed(
