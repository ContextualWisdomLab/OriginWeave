...
class BrowserSessionRustdocLibraryPathInputAuthorityContractTests(unittest.TestCase):
    """Keep rustdoc dependency search paths inside reviewed documentation provenance."""
    ...
    def _assert_external_input_fails_closed(self, config_text: str) -> None:
        root = self._workspace_with_config(config_text)
        with self.assertRaisesRegex(AssertionError, "rustdocflags:external link input"):
            authority._assert_no_repository_cargo_compiler_execution_overrides(root)

    def test_build_rustdocflags_library_path_fails_closed(self) -> None:
        self._assert_external_input_fails_closed(
            '[build]\nrustdocflags = ["--library-path", "tools/review-bypass-deps"]\n'
        )

    def test_target_rustdocflags_equals_library_path_fails_closed(self) -> None:
        self._assert_external_input_fails_closed(
            "[target.'cfg(unix)']\nrustdocflags = [\"--library-path=tools/review-bypass-deps\"]\n"
        )

    def test_unrelated_rustdocflag_remains_allowed(self) -> None:
