...
class BrowserSessionLinkerDriverSpecsContractTests(unittest.TestCase):
    """Keep GCC driver spec-file authority inside the reviewed Browser Session execution boundary."""
    ...
    def test_repository_driver_specs_file_fails_closed(self) -> None:
        root = self._workspace_with_flags(
            '["-C", "link-arg=-specs=tools/review-bypass.specs"]'
        )
    ...
    def test_repository_driver_specs_inside_link_args_fails_closed(self) -> None:
        root = self._workspace_with_flags(
