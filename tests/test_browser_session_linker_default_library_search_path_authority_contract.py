...
class BrowserSessionLinkerDefaultLibrarySearchPathAuthorityContractTests(unittest.TestCase):
    """Keep GNU ld default-library search-path overrides in reviewed provenance."""
    ...
    def test_build_rustflags_compact_y_search_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=-Wl,-Ytools/shadow-libs"]\n',
            "rustflags:codegen linker",
        )

    def test_target_rustflags_compact_y_search_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=--for-linker=-Ytools/target-libs\"]\n",
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_compact_y_search_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,-Ytools/doc-libs"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_forwarded_compact_y_search_path_fails_closed(self) -> None:
        self._assert_fails_closed(
    ...
    def test_relro_toggle_remains_allowed(self) -> None:
