...
class BrowserSessionRustdocExternalInputContractTests(unittest.TestCase):
    """Keep rustdoc-selected external crate and native-library inputs reviewed."""
    ...
    def test_build_rustdocflags_extern_input_fails_closed(self) -> None:
        self._assert_external_input_fails_closed(
            '[build]\nrustdocflags = ["--extern", "review_bypass=tools/libreview_bypass.rlib"]\n'
        )

    def test_target_rustdocflags_library_search_path_fails_closed(self) -> None:
        self._assert_external_input_fails_closed(
