...
class BrowserSessionLinkerPositionalInputContractTests(unittest.TestCase):
    """Keep positional native link inputs inside the reviewed Browser Session provenance boundary."""
    ...
    def _assert_positional_input_fails_closed(self, config_text: str) -> None:
    ...
    def test_direct_positional_object_link_arg_fails_closed(self) -> None:
        self._assert_positional_input_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=tools/review-bypass-object.o"]\n'
        )

    def test_target_positional_archive_link_args_fails_closed(self) -> None:
        self._assert_positional_input_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-args=tools/review-bypass-archive.a\"]\n"
        )

    def test_forwarded_positional_object_fails_closed(self) -> None:
        for forwarded in (
            "-Wl,tools/review-bypass-object.o",
            "--for-linker=tools/review-bypass-object.o",
            "-Xlinker tools/review-bypass-object.o",
        ):
