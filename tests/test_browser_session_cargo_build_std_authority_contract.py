...
class BrowserSessionCargoBuildStdAuthorityContractTests(unittest.TestCase):
    """Keep repository-selected standard-library source builds outside the Browser Session TCB."""
    ...
    def test_repository_build_std_source_selection_fails_closed(self) -> None:
        for config_text in (
    ...
    def test_repository_build_std_features_fails_closed(self) -> None:
    ...
    def test_unrelated_unstable_setting_remains_allowed(self) -> None:
        root = self._workspace_with_config('[unstable]\nmtime-on-use = true\n')
        authority._assert_no_repository_cargo_compiler_execution_overrides(root)
