...
class BrowserSessionLinkerRuntimeFilterAuthorityContractTests(unittest.TestCase):
    """Keep ELF DT_AUXILIARY/DT_FILTER runtime implementation selection in reviewed provenance."""
    ...
    def test_build_rustflags_auxiliary_library_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=-Wl,--auxiliary=tools/review-bypass-impl.so"]\n',
            "rustflags:codegen linker",
        )

    def test_target_rustflags_filter_library_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=--for-linker=--filter=tools/review-bypass-impl.so\"]\n",
            "rustflags:codegen linker",
        )

    def test_single_dash_long_auxiliary_alias_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "link-arg=-Wl,-auxiliary=tools/review-bypass-impl.so"]\n',
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_compact_filter_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,-Ftools/review-bypass-impl.so"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_forwarded_compact_auxiliary_fails_closed(self) -> None:
        self._assert_fails_closed(
