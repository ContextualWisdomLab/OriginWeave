...
class BrowserSessionDistributedThinLtoExecutionAuthorityContractTests(unittest.TestCase):
    """Keep LLD DTLTO distributor/compiler executables outside reviewed Browser Session builds."""
    ...
    def test_build_rustflags_dtlto_distributor_fails_closed(self) -> None:
    ...
    def test_target_rustflags_dtlto_remote_compiler_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=-Wl,--thinlto-remote-compiler=tools/review-bypass-compiler\"]\n",
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_dtlto_distributor_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,--thinlto-distributor=tools/review-bypass-distributor"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_forwarded_dtlto_remote_compiler_fails_closed(self) -> None:
        self._assert_fails_closed(
    ...
    def test_non_executable_thinlto_job_count_remains_allowed(self) -> None:
