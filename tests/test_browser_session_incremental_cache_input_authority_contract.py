...
class BrowserSessionIncrementalCacheInputAuthorityContractTests(unittest.TestCase):
    """Keep repository-selected incremental cache state inside reviewed compiler provenance."""
    ...
    def test_build_rustflags_incremental_cache_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "incremental=tools/review-bypass-incremental"]\n'
        )

    def test_target_rustflags_compact_incremental_cache_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-Cincremental=tools/review-bypass-incremental\"]\n"
        )

    def test_profile_rustflags_incremental_cache_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[unstable]\nprofile-rustflags = true\n\n[profile.release]\nrustflags = ["--codegen=incremental=tools/review-bypass-incremental"]\n'
        )

    def test_build_rustdocflags_incremental_cache_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--codegen=incremental=tools/review-bypass-incremental"]\n'
        )

    def test_doctest_build_arg_incremental_cache_path_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--doctest-build-arg=-C", "--doctest-build-arg=incremental=tools/review-bypass-incremental"]\n'
        )

    def test_cargo_managed_incremental_toggle_remains_allowed(self) -> None:
        directory, root = _workspace_with_config('[build]\nincremental = false\n')
        self.addCleanup(directory.cleanup)
        authority._assert_no_repository_cargo_compiler_execution_overrides(root)
