...
class BrowserSessionLldErrorHandlerAuthorityContractTests(unittest.TestCase):
    """Keep linker-selected error-handler executables outside reviewed Browser Session builds."""
    ...
    def test_build_rustflags_lld_error_handler_script_fails_closed(self) -> None:
    ...
    def test_target_rustflags_split_lld_error_handler_script_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=-Wl,--error-handling-script,tools/review-bypass-handler\"]\n",
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_lld_error_handler_script_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,--error-handling-script=tools/review-bypass-handler"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_forwarded_lld_error_handler_script_fails_closed(self) -> None:
        self._assert_fails_closed(
    ...
    def test_non_executable_linker_policy_remains_allowed(self) -> None:
