...
class BrowserSessionLlvmArgsAuthorityContractTests(unittest.TestCase):
    """Keep repository-selected direct LLVM option authority outside the reviewed build TCB."""
    ...
    def test_build_rustflags_llvm_args_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustflags = ["-C", "llvm-args=-debug-pass=Structure"]\n'
        )

    def test_target_rustflags_compact_llvm_args_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-Cllvm-args=-print-after-all\"]\n"
        )

    def test_profile_rustflags_long_llvm_args_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[unstable]\nprofile-rustflags = true\n\n[profile.release]\nrustflags = ["--codegen=llvm-args=-debug-pass=Structure"]\n'
        )

    def test_build_rustdocflags_llvm_args_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--codegen", "llvm-args=-debug-pass=Structure"]\n'
        )

    def test_doctest_build_arg_llvm_args_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["--doctest-build-arg=-C", "--doctest-build-arg=llvm-args=-debug-pass=Structure"]\n'
        )

    def test_typed_codegen_option_remains_allowed(self) -> None:
