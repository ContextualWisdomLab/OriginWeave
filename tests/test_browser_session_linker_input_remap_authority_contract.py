...
class BrowserSessionLinkerInputRemapAuthorityContractTests(unittest.TestCase):
    """Keep linker input-remapping policy from replacing reviewed Browser Session link inputs."""
    ...
    def test_build_rustflags_inline_remap_fails_closed(self) -> None:
    ...
    def test_target_rustflags_remap_file_equals_fails_closed(self) -> None:
        self._assert_fails_closed(
            "[target.'cfg(unix)']\nrustflags = [\"-C\", \"link-arg=--for-linker=--remap-inputs-file=tools/review-bypass-remaps.txt\"]\n",
            "rustflags:codegen linker",
        )

    def test_build_rustdocflags_single_dash_remap_file_fails_closed(self) -> None:
        self._assert_fails_closed(
            '[build]\nrustdocflags = ["-C", "link-arg=-Wl,-remap-inputs-file=tools/review-bypass-remaps.txt"]\n',
            "rustdocflags:codegen linker",
        )

    def test_doctest_forwarded_single_dash_inline_remap_fails_closed(self) -> None:
        self._assert_fails_closed(
    ...
    def test_non_input_linker_hardening_option_remains_allowed(self) -> None:
