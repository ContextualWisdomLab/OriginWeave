...
OriginWeave records dated standards claims from the immutable W3C URI itself. The mutable `https://www.w3.org/TR/webdriver-bidi/` latest-publication URI may be useful as navigation metadata, but its observed state is not used to negate or silently redefine an immutable dated publication. Publication freshness remains separate from runtime qualification: this acceptance branch does not repin or qualify the adapter merely because a dated draft exists.
...
A newer navigation in the same live context supersedes an older pending witness even when that older witness has already recorded one non-terminal `navigationCommitted` progress transition. It also supersedes any unused re-establishment opportunity from the older generation without spending an epoch. Delayed evidence from the superseded witness must not be remapped into the newer navigation. A committed-but-unsettled predecessor may not retain hidden progress state capable of consuming the newer navigation's closure or eligibility. Sibling contexts remain independent: a pending navigation in B must not erase a valid re-establishment opportunity in A, and A may re-establish while B remains fail-closed pending its own qualified closure. Commit progress is likewise ownership-local: destroying sibling A after both A and B have accepted commit progress must not clear B's already-consumed commit slot or unlock B before B's own qualified closure. The inverse cleanup boundary is also ownership-local: once A has earned an unused positive, typed-negative, or download-derived re-establishment opportunity, later destruction of sibling B must not erase A's opportunity, reactivate A's retained authority, reset the aggregate epoch, or require any compensating protocol event before A can re-establish.
...
- **Duplicate commit progress:** after the first qualified `navigationCommitted`, a second `navigationCommitted` for the same witness is rejected before mutation. Rejection must preserve lifecycle/recovery state, keep retained authority revoked, perform zero adapter I/O, spend no epoch, create no re-establishment eligibility, and leave the original witness pending for exactly one later qualified closure.
...
- **Committed-then-superseded:** `navigationStarted(A) → navigationCommitted(A) → navigationStarted(B)` kills A's committed-but-unsettled witness. A later duplicate commit, positive closure, negative closure, or `downloadWillBegin(A)` is permanently stale, must fail before lifecycle/recovery/authority/eligibility/epoch/I/O mutation, and must leave B as the sole pending navigation generation.
...
- **Context-local destruction:** proven destruction consumes only the destroyed context's ownership, pending witness, commit-progress bookkeeping, and unused re-establishment opportunity. It must not clear or settle a sibling pending navigation, synthesize recovery evidence for the sibling, or spend a presentation epoch.
- **Committed-sibling destruction:** if A and B have each already accepted their first `navigationCommitted`, proven destruction of A must not clear B's consumed commit-progress slot. A late commit remains ownership-invalid; a duplicate B commit remains stale and non-mutating; B stays revoked and ineligible until B's own positive/negative/download closure, after which exactly one executable fresh authority may be minted.
- **Eligible-sibling destruction isolation:** if A has already earned one unused complete-positive, `navigationAborted`, `navigationFailed`, or `downloadWillBegin` re-establishment opportunity while B is independently committed, proven destruction of B must leave A's eligibility intact. A's retained pre-navigation authority remains revoked; destruction is the only lifecycle adapter I/O; A must be able to re-establish immediately after destruction, before any later B evidence could compensate for bad cleanup. Destroyed-B evidence remains ownership-invalid and cannot disturb A's fresh authority.
...
Required real-browser scenarios include commit-progress followed by positive, negative, and download-start closure; duplicate commit replay while the witness remains pending; committed A superseded by B before A closure followed by delayed A commit/positive/negative/download evidence; replay before and after re-establishment; overlapping same-context navigation; sibling-context independence; destruction while another context is pending; destruction of A after both sibling navigations have committed while B preserves duplicate-commit rejection and remains locked until its own closure; destruction of committed B while sibling A holds an unused positive/negative/download-derived re-establishment opportunity, with A re-establishing immediately after B destruction; raw-id ABA reuse; trust loss; crash/cleanup; and the ability of each newly minted authority to perform its intended current-context browser mutation.
