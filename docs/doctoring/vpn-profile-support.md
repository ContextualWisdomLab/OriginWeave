...
OriginWeave intentionally supports a bounded subset of WireGuard/wg-quick-style profile text rather than claiming lossless or complete wg-quick compatibility. In `[Interface]`, the admitted keys are `Address`, `DNS`, `MTU`, `ListenPort`, and `PrivateKey`. In `[Peer]`, the admitted keys are `PublicKey`, `PresharedKey`, `Endpoint`, `AllowedIPs`, and `PersistentKeepalive`. Blank lines, full-line `#` comments, and trailing `#` comments are ignored for this WireGuard grammar. Unknown keys fail closed. `PreUp`, `PostUp`, `PreDown`, `PostDown`, `SaveConfig`, and `Table` are explicitly rejected because importing them would retain command, persistence, or host-routing authority that belongs to a later privileged adapter. Representative exported-style normalization is exercised by `crates/originweave-vpn-profile/tests/wireguard_inline_comments.rs`, including an interface, peer, DNS server/search-domain split, host-address normalization, default-route intent, endpoint intent, and inline comments.

For admitted WireGuard text, the crate completes a side-effect-free validation pass before raw `PrivateKey` or `PresharedKey` material reaches the caller-supplied trusted secret importer. During the real import pass, OriginWeave journals every returned opaque reference. If a later import or profile-construction step fails, it invokes `discard_secret` for those references in reverse order. Successful reverse-order rollback preserves the original typed failure; any cleanup failure returns `SecretCleanupFailed` so incomplete cleanup cannot be mistaken for a clean rejection. The normalized result cannot execute profile directives and does not itself create interfaces or routes.
