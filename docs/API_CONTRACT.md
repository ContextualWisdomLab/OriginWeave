...
A full protected field is not eligible for model input merely because sensitive-data policy and a reviewed model route both authorize it. The trusted broker/orchestrator must first derive current necessity from executable alternatives. Necessity evidence carries an **exclusive trusted-time validity horizon**: zero-lifetime evidence is invalid, and evidence evaluated at or after `valid_until` is expired and cannot authorize disclosure. If an opaque handle, deterministic transform, local rule, structured tool, or approved derived value can satisfy the task, the policy boundary returns a typed lower-disclosure-path denial rather than full-field model authorization. A fresh `no lower disclosure path` assertion is still metadata, not self-authenticating proof: untrusted page/model content cannot mint it, the trusted broker must use the same trusted clock domain as policy evaluation, and protected bytes remain resolved only inside the trusted value boundary after policy and lifecycle revalidation.
