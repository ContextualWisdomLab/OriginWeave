...
A full protected field is not eligible for model input merely because sensitive-data policy and a reviewed model route both authorize it. The trusted broker/orchestrator must first derive current necessity from executable alternatives for the **same exact sensitive-data request**. Necessity evidence is bound to the request's tenant, task, field, purpose, canonical destination, and classification tuple; evidence derived for another request fails closed with typed `NecessityAuthorityMismatch` and cannot be replayed into an otherwise authorized model disclosure. The existing sensitive disclosure and reviewed invocation authority are evaluated first, so necessity metadata cannot upgrade a denied, mismatched, or expired base policy. Only after those boundaries authorize does OriginWeave verify the necessity-request binding and the evidence's **exclusive trusted-time validity horizon**: zero-lifetime evidence is invalid, and evidence evaluated at or after `valid_until` is expired and cannot authorize disclosure. If an opaque handle, deterministic transform, local rule, structured tool, or approved derived value can satisfy the task, the policy boundary returns a typed lower-disclosure-path denial rather than full-field model authorization. A fresh request-bound `no lower disclosure path` assertion is still metadata, not self-authenticating proof: untrusted page/model content cannot mint it, the trusted broker must derive it from current executable state using the same trusted clock domain as policy evaluation, and protected bytes remain resolved only inside the trusted value boundary after policy and lifecycle revalidation.
