...
| PRD-OBS-002 | Actionable semantic-node handles are invalidated by relevant document-epoch changes at the action linearization boundary | Accepted architecture | Core `SameDocumentMutationKind` now decides when a same-document mutation must rotate `DocumentEpoch` so a replaced checkout field cannot keep its old handle; live MutationObserver/BiDi invalidation and atomic dispatch remain planned adapter work; active PR #40 owns the bounded protocol-ID registry and remains non-shipped evidence |
