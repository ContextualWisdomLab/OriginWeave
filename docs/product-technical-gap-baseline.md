...
- The next Browser Session production gaps after create-attempt correlation are a purpose-bounded same-adapter `RecoveryRequired`/`TransportLost` handoff and bounded hot ownership/history separation. Durable crash/process-restart persistence remains separate; `abandoned_bound_session_count()` is process-local operability evidence, not destruction proof or durable recovery storage.
...
1. Finish #317's Browser Session prerequisite in its single-writer lane: exact create-attempt recovery correlation is implemented; next prove the same-adapter recovery handoff and bounded hot ownership/history split, then run the complete exact-head contract/format/test/Clippy/rustdoc/100%-coverage/review sequence. Do not begin #318/#321 production implementation in parallel.
2. After #317 is verified, ordinary non-force restack #318 and #321 while preserving every valid acceptance delta; then restack #316 and implement BiDi pending→accepted/quarantined correlation and remote-liveness reconciliation without moving Browser Session authority into the adapter.
