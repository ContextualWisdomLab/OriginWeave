...
Observed at (UTC): `2026-09-05T05:39:00Z`.
...
- A predecessor #238 head materialized native CI run `33945915264`; both `ubuntu-24.04` jobs were queued without an assigned runner. Across the latest 100 repository workflow runs, 22 were queued and none was in progress, with the oldest queued run created at `2026-09-05T04:20:26Z`. This is runner-admission evidence, not a code failure or passing check; rerunning would only add duplicate queue load. The moving PR's exact current head remains live-metadata-only to avoid a non-convergent self-reference.
