...
Observed at (UTC): `2026-09-04T12:41:17Z`.
...
- Issue #279 owns the remaining documentation-CI partitioning gap. PR #282 is Draft at exact head `b2a120f892973e76c0ea0f06e7105bdf7a268009`; its classifier now requires complete 40-character SHA-1 or 64-character SHA-256 raw-diff object identities. The current test-only head also demonstrates that rename/copy similarity scores still need binding to blob identity before production is complete. Git-canonical paths and contributor/agent-instruction authority remain fail-closed. An authorized non-scheduled workflow owner must connect a trusted protected-base/immutable-SHA classifier to exact base/head raw evidence rather than execute mutable PR-head classification authority.
