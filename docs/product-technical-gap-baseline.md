...
| Stacked HTTP `Content-Encoding` interoperability without budget bypass | Canonical HTTP parent #37 remains exact `219b43bfa87ab4fd90a77bb962df2797925bf661`. Active successor #327 is exact `7e25dc4ec1840b48fd38099dbc19ac89f776ce1c`, directly stacked **29 ahead / 0 behind**. It carries bounded depth-2 `gzip`/`deflate` decoding, original-coded cumulative expansion accounting, exact-layer raw-DEFLATE compatibility, RFC 9530 pre-decode digest handling, ordered per-layer evidence, Proposed ADR 0011 currentization, and an authenticated-TLS buyer-path performance instrument. The instrument still runs 31 measured samples for representative `gzip, deflate` and `deflate, gzip` profiles and fails above p95 **20,000 µs**. Receipt provenance remains source/environment bound through `ORIGINWEAVE_PERFORMANCE_SOURCE_REVISION` and mandatory `ORIGINWEAVE_PERFORMANCE_ENVIRONMENT_ID`, with runtime OS/architecture/parallelism emitted on every profile line. The current repair separates `budget_status` from `acceptance_status`: `GITHUB_SHA` fallback measurements are marked `UNACCEPTED_SOURCE_FALLBACK` and the process exits nonzero for acceptance unless the exact source revision is supplied explicitly. The follow-up `7e25dc4e...` repairs the unit-test parent-constant reference without changing measurement semantics. Exact-head CI `35950201462` is still pre-step queued, so this is **not an accepted performance receipt** and no ≤20 ms claim is made. | #326/#327 remain the OriginWeave HTTP owner path. Preserve #37 as the canonical parent; require exact-head repository/security/CodeQL/review acceptance on #327 plus an actually executed receipt whose explicit source revision matches the intended measured head and whose `budget_status=PASS` and `acceptance_status=PASS` on an identified environment before ordinary parent-first integration. Do not call the current Vec-based bounded implementation streaming and do not move decoding or policy authority into Chromium, MCP, a model, or another CWL owner. | **OPEN — ACTIVE SUCCESSOR** |
