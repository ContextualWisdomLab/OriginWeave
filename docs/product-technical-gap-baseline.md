...
## Observed delivery cut — 2026-09-09 12:57 UTC

This is a dated observation receipt, not a continuously live counter. The cut below includes this #309 Draft successor after its Ready-to-Draft repair. Live GitHub state supersedes this cut after 2026-09-09 12:57 UTC.

- Protected-main truth remains signed-valid `87c4daa1830bac5a5228b6036752ad5633232085`.
- The observed queue contains **130 open pull requests: 12 Ready/non-draft and 118 Draft; 14 open non-PR issues**. The three GitHub searches used for total/Ready/Draft counts and the issue search all returned `incomplete_results=false`.
- Open issues: the 14 non-PR items above remain separate from the 130-PR delivery queue and are not counted as shipped product behavior.
...
| Buyer decision surface remains usable while evidence history stays immutable | The previous baseline blob is 186,500 bytes and includes repeated dated checkpoints before the commercial gap surface. This successor preserves that exact blob under `docs/evidence/` and keeps this path bounded by regression. | #238 documentation owner: dated observation receipts and the buyer matrix must remain reachable near the top; detailed runbooks belong in immutable evidence/traceability receipts. | **IN REPAIR** |
| Protected merge/release evidence | Active rules require counted approval and central workflows. Current Release inventory is empty. | Normal protected-main merge only after exact-head checks/reviews. Release-ready head must then re-read tag/package state and produce version/CHANGELOG, signed artifact, SBOM, provenance, reproducibility and rollback evidence before immutable release. | **OPEN** |
