...
- **Bounded-context correction:** active PR #272 moves MCP protocol contracts from `originweave-core` to `originweave-mcp` without changing routing or authorization semantics
...
- adds a machine-checkable repository fitness test preventing MCP protocol code from returning to core or policy from depending outward on MCP.
...
- `crates/originweave-mcp/src/lib.rs` — adapter-owned `evaluate_mcp` bridge that delegates to `originweave_policy::evaluate` only after route/action equality;
