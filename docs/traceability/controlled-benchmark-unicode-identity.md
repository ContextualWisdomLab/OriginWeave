...
|---|---|---|---|---|
| Reject Unicode default-ignorable scalars from benchmark-owned run-context identity | UAX #31 Rev. 45 §7.3; formally released Unicode 18.0.0 versioned `DerivedCoreProperties.txt` | `CONTROLLED_BENCHMARK_UNICODE_IDENTITY_PROFILE`; `is_unicode_18_default_ignorable`; `validate_run_context_field` | `unicode_18_default_ignorable_reproducibility_context_fails_closed` | Active PR #324 only; raw artifact receipt pending #325 |
| Bind the property set to a named Unicode version | UAX31-C1 requires version identification; DICP has no general stability guarantee | profile value `unicode-18.0.0-default-ignorable-exclusion`; exact local range table | profile-string assertion plus property-range boundary cases | Grammar pinned; formal release, property equivalence, and normalized digest confirmed; raw-file receipt pending #325 |
| Reconcile the pre-release observation after formal publication | Unicode Consortium release registry and 2026-09-16 release announcement; Version 18.0.0 formal citation; direct versioned UCD artifact | no silent table rewrite; #325 preserves raw-byte receipt requirement | direct 27-entry / 4,174-scalar comparison; normalized source/implementation digest equality | Formal finality + property-set equivalence + normalized digest satisfied; raw byte length/full-file SHA-256 open |
...
A deterministic normalized property receipt makes that equivalence independently reproducible. Expand the DICP source entries to the ascending scalar set; encode each scalar as six uppercase hexadecimal digits followed by LF (`%06X\n`); concatenate all records. The 4,174-scalar stream is **29,218 bytes** with SHA-256 `673264e62183e35f6055a2ad4940403e706669e0750fcc5d56a99f158fb3bb93`. Expanding #324's 17 compressed implementation ranges under the same normalization yields the same byte stream and digest. This is a property-set digest, not the raw UCD file digest.
...
#325 no longer waits for the stale summary-page banner to disappear and no longer lacks property equivalence or a normalized-property digest. The remaining provenance gap is the official versioned artifact's **raw byte length and full-file SHA-256**. If that receipt confirms the same artifact, traceability can close provenance without changing admission semantics. If the artifact unexpectedly resolves to different content or a later normalized-property comparison differs, the table must not be silently rewritten under the existing evidence identity: a fresh hostile/compatibility RED and a new profile identity are required.
