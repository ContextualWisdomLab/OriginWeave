...
|---|---|---|---|---|
| Reject Unicode default-ignorable scalars from benchmark-owned run-context identity | UAX #31 Rev. 45 §7.3; stable Unicode 18.0.0 DICP property set | `CONTROLLED_BENCHMARK_UNICODE_IDENTITY_PROFILE`; `is_unicode_18_default_ignorable`; `validate_run_context_field` | focused `unicode_18_default_ignorable_reproducibility_context_fails_closed`; exhaustive `every_unicode_18_default_ignorable_source_scalar_fails_closed` over all 27 stable source ranges / 4,174 scalars | Active PR #324 only; exact-head repository acceptance still pending |
