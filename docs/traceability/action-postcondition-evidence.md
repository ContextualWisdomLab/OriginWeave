...
Security review `5098613778` found that the Agent Task lane had been launched with Chromium `--no-sandbox`, so predecessor browser success is compatibility evidence rather than production-security evidence. The test-only sandbox contract then failed against the unchanged launch source. Exact repair commit `60b697095be510a129cfb61a3fd97790cf7a0679` removes only the Agent Task lane's `--no-sandbox` argument; the separate MV3 compatibility pass is intentionally unchanged. The focused source contract now passes, but a sandbox-enabled pinned-Chromium exact-head run is still required before this active PR can claim repaired browser security evidence.
