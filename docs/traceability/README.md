...
| BAP lifecycle retries remain explicitly identifiable | IMPLEMENTED_ON_ACTIVE_PR | issue #200; active `originweave-bap` lane | The in-memory command receipt binds a bounded idempotency key and task identifier to one accepted transition; durable deduplication and side-effect suppression remain Planned |
