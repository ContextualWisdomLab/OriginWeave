...
Every action request carries an immutable lowercase `sha256:` digest of the complete canonical action intent (National Institute of Standards and Technology, 2015). R3 and R4 approval evidence is bound to the exact action kind, target origin, and intent digest. Approval for one payload cannot authorize another payload even when both use the same action kind and origin.
...
Web Hypertext Application Technology Working Group. (2026, September 10). *URL standard*. https://url.spec.whatwg.org/
