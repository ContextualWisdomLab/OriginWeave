...
- `scripts/release/validate_spdx_jsonld.py` consumes actual bytes for a narrow envelope check. It accepts at most 16 MiB, requires strict UTF-8 JSON, performs a string-aware lexical preflight before `json.loads`, caps opening JSON containers at 524,288 and total structural `[]{}:,` tokens at 1,048,576, requires exactly the top-level `@context` and `@graph` members, requires `@context` to equal the exact SPDX 3.0.1 global-context string, requires exactly one top-level `SpdxDocument`, and rejects any additional nested `SpdxDocument`. Context arrays and inline/remote context extensions fail closed at this stage.
- The lexical preflight counts structural punctuation only outside JSON strings. It is a resource-admission check, not a substitute JSON grammar; `json.loads` remains responsible for syntax after the preflight. The 524,288-container and 1,048,576-structure-token thresholds are provisional OriginWeave resource-policy ceilings rather than SPDX semantic limits.
