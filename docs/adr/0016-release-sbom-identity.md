...
- Empty, duplicate, foreign, and case-drifted artifact identities must fail closed.
...
- The SBOM artifact is represented by the existing manifest-backed `ReleaseArtifact`, including its lowercase SHA-256 identity.
...
Selected. The SBOM artifact and all described artifact names must resolve exactly within the same release manifest, duplicates and empty sets fail closed, and described identities are stored in deterministic lexical order.
...
4. Every described artifact name must exactly match an artifact admitted by the same manifest.
5. Duplicate described artifact names fail closed.
6. Case-drifted names are not normalized and fail as unknown unless they exactly match the preserved manifest spelling.
7. Described artifact names are stored in deterministic lexical order.
8. The manifest's existing bounded artifact inventory transitively bounds the binding.
9. Validation failures remain typed, deterministic standard Rust errors without hidden fallback.
10. Constructing the binding does not claim SPDX content conformance, generation completeness, artifact authentication, provenance, signatures, publication, installation, update, rollback, or release authority.

## Consequences

Later SPDX tooling receives one explicit join point between SBOM identity and the exact release inventory. It cannot silently describe an artifact that the release manifest did not admit, and buyers can retain the manifest's existing digest identity for the SBOM artifact itself.
...
A missing SBOM artifact, empty described-artifact set, foreign described artifact, duplicate described identity, or case-drifted identity fails closed before a binding is produced. No fallback rewrites a name, drops an artifact, substitutes another manifest entry, or converts an invalid binding into success.
...
The boundary reduces supply-chain identity ambiguity by preventing the SBOM layer from creating a second ambient artifact namespace. It introduces no secrets, personal data, signing material, privileged network access, reviewer authority, or release authority. Scheduled development agents remain unable to merge, sign, tag, publish, install, or change repository governance.

## Tests and acceptance evidence

The owning `originweave-core` tests must prove exact manifest-backed SBOM admission, deterministic described-artifact ordering, and fail-closed behavior for missing SBOM identity, empty described sets, foreign artifacts, duplicates, and case drift. Public error text and `std::error::Error` behavior are part of the deterministic contract.
