...
Opaque handle use is separately bound to tenant, task, field, purpose, canonical destination, data classification, exclusive expiry time, and maximum use count. A field reclassification therefore invalidates the prior handle authority even when every other identifier is unchanged; the caller must obtain a newly authorized handle for the new classification. `evaluate_handle_use` is intentionally a pure admission predicate: it compares exact authority, classification, trusted-time input, and the caller-supplied `uses_so_far` is untrusted at this function boundary. It does not independently enforce the maximum-use limit, own mutable handle state, resolve a handle, consume a use, authenticate a workload audience, or return the protected value. Only a trusted broker or `SensitiveHandleUseState` may treat its result as authorization; that boundary must obtain caller-unforgeable count and time and atomically reserve a use.
