...
`SensitiveHandleUseState` is the bounded in-process stateful reservation primitive. It owns the current reserved-use count and a monotonic trusted-time floor for its exact handle scope. Scope equality is evaluated before the trusted-time floor: a mismatched authority always fails as `ScopeMismatch` without reading or mutating that floor, so foreign authority cannot probe whether a newer trusted time has been observed or poison the state by presenting an arbitrarily advanced timestamp. For an exact-scope reservation attempt, a trusted time that predates the latest exact-scope time already observed by the state fails closed as `TrustedTimeRollback`. Every non-rollback exact-scope trusted time advances the floor even when expiry or use-count policy later denies the attempt, preventing a stale clock value from restoring authority after a later exact-scope time has already been observed. Denied reservations never consume a use; only an `Authorized` reservation increments the authoritative count.
...
- A caller with the wrong tenant, task, field, purpose, destination, or classification cannot reuse another disclosure scope or opaque handle, cannot advance that handle state's trusted-time floor, and cannot probe whether the floor has already moved forward.
...
- An expired or exhausted opaque handle fails closed, and an earlier exact-scope trusted-time value cannot restore authority after a later exact-scope trusted time has already been observed by `SensitiveHandleUseState`.
