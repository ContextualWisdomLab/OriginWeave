...
Opaque handle use is separately bound to tenant, task, field, purpose, canonical destination, data classification, a non-transferable audience identifier, exclusive expiry time, and maximum use count. Audience identifiers use the same 1–128 byte bounded ASCII policy-token grammar as the other authority identifiers. A field reclassification or audience change therefore invalidates the prior handle authority even when every other identifier is unchanged; the caller must obtain a newly authorized handle for the new classification or audience. `evaluate_handle_use` is intentionally a pure admission predicate: it compares authority, classification, audience, trusted-time input, and broker-recorded prior-use count, but it does not authenticate the audience, own mutable handle state, resolve a handle, consume a use, or return the protected value. An arbitrary caller-supplied audience string is not identity evidence and must never be treated as standalone enforcement.

The trusted broker or browser adapter is the stateful enforcement and audience-authentication boundary. Before resolving any protected value, it must authenticate the presenting workload or service through its trusted identity boundary, derive the exact handle audience from that authenticated identity, obtain trusted time and authoritative caller-unforgeable handle state, atomically compare the exact scope, classification, audience, exclusive expiry, and current use count, and reserve or increment the use count in the same transaction that grants the use. Concurrent or replayed requests therefore compete for one authoritative count rather than reusing a stale caller-supplied count, and possession of a handle by a different workload does not transfer its authority. Once a use has been successfully reserved, a downstream browser/action failure does not silently refund that use unless a separately specified compensating transaction is both safe and auditable. At the expiry boundary (`now >= expires_at`) no new reservation is permitted. Immediately before release, the broker rechecks that the reserved handle, authenticated audience, requested scope, and classification still match and that revocation or lifecycle state has not invalidated the disclosure.
...
- A caller with the wrong tenant, task, field, purpose, destination, classification, or authenticated audience cannot reuse another disclosure scope or opaque handle.
...
- Missing, oversized, whitespace-bearing, control-bearing, Unicode, or otherwise malformed tenant, task, field, purpose, or audience identifiers cannot become authority through equality with another invalid scope; destination validity is guaranteed by the canonical `Origin` boundary.
- A stale, reclassified, wrong-audience, or exhausted opaque handle fails closed in the pure predicate, while the future broker must authenticate the audience and enforce classification, audience, expiry, and use-count consumption atomically before value resolution.
...
### Transferable bearer-style handles

Rejected because possession by a different workload, connector, or adapter would otherwise be enough to replay the handle even though that presenter was never authorized for the disclosure. The trusted broker must authenticate the presenter and bind use to the exact audience recorded when the handle is authorized.
...
Tests must prove exact-scope disclosure, canonical destination behavior, denial on every authority-dimension mismatch, fail-closed behavior for missing or malformed authority, acceptance at the exact 128-byte identifier bound, rejection beyond that bound, rejection of whitespace/control/Unicode identifiers, every supported disclosure result, opaque-handle classification mismatch, exact audience match, wrong and malformed audience rejection, audience acceptance at exactly 128 bytes and rejection beyond that bound, expiry, use-count exhaustion, and destination mismatch. The broker slice must add authenticated-audience derivation, classification-change, audience-transfer, concurrency, replay, expiry-boundary, post-reservation failure, revocation, and atomic compare-and-increment tests before any protected-value resolution is described as implemented. Production function, line, region, and branch coverage remains exactly 100%.
