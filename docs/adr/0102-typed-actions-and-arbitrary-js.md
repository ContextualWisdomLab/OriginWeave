...
The policy engine receives typed requests from trusted control-plane code. Page-derived selectors, values, and observations remain untrusted parameters until validated. Browser adapters may translate a typed action into lower-level protocol calls, but the adapter cannot grant a new action kind or expand its declared authority.
...
Typed actions reduce prompt-injection and confused-deputy blast radius by constraining what model output can request. Secret delivery remains opaque-handle based. High-risk actions preserve governed approval, and evidence can record parameters after credential-safe redaction.

## Tests and acceptance evidence

Require a table-driven mapping from action kind to capability, mutability, risk, secret usage, and post-condition requirements; tests that web content cannot create trusted instructions; adapter tests proving no arbitrary-script fallback; hostile selector/value tests; and end-to-end evidence that denied or unsupported actions cause no side effect.
