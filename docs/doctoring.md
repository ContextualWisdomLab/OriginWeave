...
Article 5 of Regulation (EU) 2016/679 establishes purpose limitation and data minimisation principles. OriginWeave uses those principles as engineering constraints—not as a claim of legal compliance—by binding each disclosure decision to an explicit business purpose and field, then denying mismatched tenant, task, field, purpose, canonical destination `Origin`, or classification authority. The first kernel carries no protected value bytes. Approval-required outcomes are control states rather than execution permissions: the required approval evidence must be obtained and the exact same scope re-evaluated before a disclosure channel can proceed.

OWASP logging guidance treats personal and other sensitive information as data that can create secondary exposure when included in application logs. OriginWeave therefore keeps raw protected values outside generic logs, traces, provenance records, and model context by default. The pure `evaluate_handle_use` contract evaluates the exact tenant/task/field/purpose/destination scope plus trusted-time and authoritative prior-use inputs but does not consume state or resolve a handle. A later trusted broker must own caller-unforgeable handle state and trusted time, atomically reserve or increment the use count before resolution, serialize concurrent and replayed requests against that authoritative count, deny new reservations at `now >= expires_at`, and recheck the reserved scope plus revocation/lifecycle state immediately before releasing a protected value. A downstream action failure does not silently refund a reserved use unless an explicit auditable compensation contract is defined.
